    ...
    */
		(RightBar.prototype.selectOptionsFromConfig = function() {
			...
			if (config) {
				...
				$('input[type=radio][name=leftsidebar-color][value=' + config.sidebar.color + ']').prop(
					'checked',
					true
				);
...
document.addEventListener('DOMContentLoaded', () => {
	...
	data.Columns.forEach((collumn) => {
		...
		th.innerHTML = collumn.DisplayName;
	...
	data.Rows.forEach((row) => {
		...
		data.Columns.forEach((collumn, index) => {
			...
			td.innerHTML = row.Cells[index] ? row.Cells[index].Value : '';
			tr.appendChild(td);
		});
